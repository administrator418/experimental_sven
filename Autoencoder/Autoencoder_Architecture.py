...
DATASET_PATH = r'C:\Users\burckhardsv\Lokale_Dateien\Schramberg\Schramberg_Datacollection\Schramberg_Datacollection\Dataset_Test'
#DATASET_PATH = r'\\srvditz1\lac\Studenten\AE_VoE_Stud\Sven Burckhard\Schramberg\Schramberg_Vortex\Dataset_512_vortex_cleaned_Schramberg_Test_mini'
...
def get_mask(image):
    ...
    if EVALUATION_METHOD == 1:
        #Safe mask for debug:
        pass
        save_path = r'C:\Users\burckhardsv\Lokale_Dateien\Schramberg\Schramberg_Datacollection\Schramberg_Datacollection\Evaluation\mask.png'
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        mask_safe = mask.copy()
        mask_safe = mask_safe*255
        cv2.imwrite(save_path, mask_safe[0, :, :, 0])

        #pass
        #save_path = f'{path_to_store_results}/{timestamp}/example_images/mask_.png'
        ...
        # mask_path = r'\\srvditz1\lac\Studenten\AE_VoE_Stud\Sven Burckhard\Figures\Gaussian_real_final_unwrapped\mask_2.png'
        # if not os.path.exists(mask_path):
        #     plt.figure(figsize=(6.67, 6.67), dpi=300)
        #     plt.switch_backend('agg')
        #     plt.imshow(mask[0, :, :, 0], cmap='gray', interpolation='none')
...
def autoencoder_split_256x256(configuration):
    ...
    down_stack = [[
        downsample(32, 4,   configuration, apply_batchnorm=False),  
        downsample(64, 4,   configuration),   
        downsample(128, 4,  configuration),  
        downsample(256, 4,  configuration),  
        downsample(256, 4,  configuration),  
        downsample(256, 4,  configuration),  
        downsample(256, 4,  configuration),  
        downsample(256, 4,  configuration),  
    ...
    up_stack = [
        upsample(512, 4,    configuration, apply_dropout=True),  
        upsample(512, 4,    configuration, apply_dropout=True), 
        upsample(512, 4,    configuration, apply_dropout=True),  
        upsample(512, 4,    configuration),  
        upsample(256, 4,    configuration),  
        upsample(128, 4,    configuration),  
        upsample(64, 4,     configuration), 
    ]
...
def autoencoder_512x512(configuration):
    ...
    if configuration['USE_SKIP_CONNECTIONS']:
        skips = reversed(skips[:-1]) # bottelneck aus den skip connection nehmen
...
def masked_mse(image1, image2, mask):
    ...
    #print(f"Original Phasemask, Minimum {tf.reduce_min(image1)}, Maximum, {tf.reduce_max(image1)}, Mean, {tf.reduce_mean(image1)}, Shape: {tf.shape(image1)}")
    #print(f"Predicted Phasemask, Minimum {tf.reduce_min(image2)}, Maximum, {tf.reduce_max(image2)}, Mean, {tf.reduce_mean(image2)}, Shape: {tf.shape(image2)}")
    ######################################################################################################################################
    if MODUS == 1:
            # Only for the prediction
            if tf.executing_eagerly():  # Ensure we are in eager execution mode
                save_path = r'\\srvditz1\lac\Studenten\AE_VoE_Stud\Sven Burckhard\Predict_Phasemask\phasemask_predicted.png'
                try:
                    image_np = image2.numpy()  # Convert the TensorFlow tensor to a NumPy array
                    #print(f"Image Shape: {image_np.shape}, Image Type: {type(image_np)}")
                    #print(f"Image Save - Min: {np.min(image_np)}, Max: {np.max(image_np)}, Mean: {np.mean(image_np)}")
                    
                    # Extract the 512x512 image from (1, 512, 512, 1)
                    image_to_save = image_np[0, :, :, 0]
                    
                    # Set pixels outside the phasemask to 128
                    mask_np = mask[0, :, :, 0]  # Extract the mask
                    #gray_value_normalized = 128 / 255.0  # Normalize the gray value to [0, 1]
                    gray_value_normalized = 0 #unwrapped phasemaks
                    image_to_save = np.where(mask_np > 0, image_to_save, gray_value_normalized)
                    
                    # Normalize the image to [0, 255]
                    image_to_save = (image_to_save * 255).astype(np.uint8)
                    #print(f"Image Save 255 - Min: {np.min(image_to_save)}, Max: {np.max(image_to_save)}, Mean: {np.mean(image_to_save)}")
                    ...
                    if success:
                        print(f"Phasemask saved to {save_path}")
                    else:
                        print(f"Failed to save phasemask to {save_path}")
...
def autoencoder_loss_calculation(autoencoder_output, target, configuration): 
    if EVALUATION_METHOD == 0:
        l1_loss = tf.reduce_mean(pixelwise_distance(target, autoencoder_output, configuration))  # MAE (L1 loss)
        l2_loss = tf.reduce_mean(tf.square(pixelwise_distance(target, autoencoder_output, configuration)))  # MSE (L2 loss)     
...
def generate_images(epoch, autoencoder, val_dataset_to_plot,training=True):
    ...
    for batch in val_dataset_to_plot:
        ...
        generated_images = autoencoder(input_images, training=training)  
        ...
        for images in zip(nearfields, farfields, generated_images, phasemasks):
            ...
            #gray_value = 128 #wrapped phasemasks
            #gray_value_noramlized = gray_value/255 # wrapped phasemasks
            gray_value_noramlized = 0 #unwrapped phasemaks
            ...
            if MODUS == 0:
                ...
                # save_path = r'\\srvditz1\lac\Studenten\AE_VoE_Stud\Sven Burckhard\Predict_Phasemask\phasemask_predicted.png'
                # image = images[2]
                # print(f"Image Save - Min: {np.min(image)}, Max: {np.max(image)}, Mean: {np.mean(image)}")
                # image_to_save = (image.numpy() * 255).astype(np.uint8)
                # print(f"Image Save 255 - Min: {np.min(image_to_save)}, Max: {np.max(image_to_save)}, Mean: {np.mean(image_to_save)}")
                # #image_original_size = resize(ORIGINAL_PHASEMASK_SIZE,image_to_save)
...
if __name__ == "__main__":
    for config_idx, configuration in enumerate(configurations):
        ...
        # Training=True für Trainingsevaluation
        for subset in ['test']:
            autoencoder_loss, l1_loss, l2_loss, ssim_score = evaluate(autoencoder, dataset[subset], configuration, training=True)
            for metric in ['autoencoder_loss', 'l1_loss', 'l2_loss', 'ssim_score']:
                log(f'final {subset} {metric} = {eval(metric)}')
                final_test_performance[metric] = eval(metric)
                print("Train")
            generate_images('final-performance', autoencoder, dataset[subset].take(configuration['BATCHES_TO_PLOT']), training=True)

        # Training=False für tatsächliche Vorhersagen
        for subset in ['test']:
            autoencoder_loss, l1_loss, l2_loss, ssim_score = evaluate(autoencoder, dataset[subset], configuration, training=False)
            for metric in ['autoencoder_loss', 'l1_loss', 'l2_loss', 'ssim_score']:
                log(f'final {subset} {metric} = {eval(metric)}')
                final_test_performance_without_dropout[metric] = eval(metric)
                print("Prediction")
            generate_images('final-performance_without_dropout', autoencoder, dataset[subset].take(configuration['BATCHES_TO_PLOT']), training=False)
