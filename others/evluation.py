...
DATASET_PATH = '/content/unwrapped_simulated_noise0.2_test/Dataset_Vortex_Simulated_Unwrapped'
...
if MODUS == 0:
    ...
    with open(f'{path_to_store_results}/configurations.txt', 'w') as f:
        ...
        f.write(
            f'Strides: {STRIDES} | Ssim evaluation metric: {EVALUATION_METHOD} | Epochs: {N_EPOCHS} | Model_number: {MODEL_NUMBER}')
...
def ssim_specific(img1, img2, mask, max_val=1.0):
    ...
    img1_masked, img2_masked = [tf.expand_dims(x, axis=-1) if x.shape.ndims == 2 else x for x in
...
def upsample(n_filters, kernel_size, configuration, apply_dropout=False):
    ...
    result.add(
        tf.keras.layers.BatchNormalization())  # TODO: why do we do this always when upsampling but only sometimes when downsampling?
...
def autoencoder_split_256x256(configuration):
    ...
    last = tf.keras.layers.Conv2DTranspose(1, configuration['KERNEL_SIZE'], strides=STRIDES, padding=PADDING,
...
def autoencoder_512x512(configuration):
    ...
    last = tf.keras.layers.Conv2DTranspose(1, configuration['KERNEL_SIZE'], strides=STRIDES, padding=PADDING,
                                           kernel_initializer=initializer,
                                           activation='sigmoid')  # (batch_size, 512, 512, 1)
    # print("Last", last)
...
def autoencoder_loss_calculation(autoencoder_output, target, configuration):  # do this also
    if EVALUATION_METHOD == 0:
        ...
        l2_loss = tf.reduce_mean(tf.square(pixelwise_distance(target, autoencoder_output,
                                                              configuration)))  # MSE (L2 loss)     # Probelm pixelwise distance uses abs()
...
def generate_images(epoch, autoencoder, val_dataset_to_plot, training=True):
    ...
    for batch in val_dataset_to_plot:
        ...
        for images in zip(nearfields, farfields, generated_images, phasemasks):
            ...
            if MODUS == 0:
                plt.savefig(
                    f'{path_to_store_results}/{timestamp}/example_images/{filename_prefix}{epoch}__img_{img_idx}.png')
            else:
                pass
            ...
            if MODUS == 0:
                ...
                plt.axis('off')
                plt.savefig(f'{path_to_store_results}/{timestamp}/example_images/phasemask_img_{img_idx}.png',
            ...
            if MODUS == 0:
                ...
                plt.axis('off')
                plt.savefig(
                    f'{path_to_store_results}/{timestamp}/example_images/predicted_phasemask__{filename_prefix}{epoch}__img_{img_idx}.png',
                    bbox_inches='tight', pad_inches=0)
                plt.close('all')
            else:
            ...
            if MODUS == 0:
                ...
                plt.axis('off')
                plt.savefig(
                    f'{path_to_store_results}/{timestamp}/example_images/difference_image__{filename_prefix}{epoch}__img_{img_idx}.png',
                    bbox_inches='tight', pad_inches=0)
                plt.close('all')
            else:
            ...
            if MODUS == 0:
                ...
                plt.axis('off')
                plt.savefig(f'{path_to_store_results}/{timestamp}/example_images/nearfield_img_{img_idx}.png',
                            bbox_inches='tight', pad_inches=0)
                plt.close('all')
            else:
                pass
            ...
            if MODUS == 0:
                ...
                plt.axis('off')
                plt.savefig(f'{path_to_store_results}/{timestamp}/example_images/farfield_img_{img_idx}.png',
                            bbox_inches='tight', pad_inches=0)
                plt.close('all')
            else:
                pass
...
def save_weights(epoch, autoencoder, delete_previous=False):
    ...
    os.makedirs(f'{path_to_store_results}/{timestamp}/checkpoints/autoencoder', exist_ok=True)
...
if __name__ == "__main__":
    for config_idx, configuration in enumerate(configurations):
        ...
        timestamp = str(datetime.datetime.now()).replace(':', '-').replace('.', '-').replace(' ', '_')
        if MODUS == 0:
            os.makedirs(f'{path_to_store_results}/{timestamp}/example_images', exist_ok=True)
            os.makedirs(f'{path_to_store_results}/{timestamp}/history', exist_ok=True)
        ...
        # Optimizer
        generator_optimizer = tf.keras.optimizers.Adam(learning_rate=configuration['LEARNING_RATE'],
        ...
        def train_step(input_images, targets, configuration):
            ...
            with tf.GradientTape() as gen_tape:
                ...
                autoencoder_loss, l1_loss, l2_loss = autoencoder_loss_calculation(autoencoder_output, targets,
                                                                                  configuration)
                ...
                try:
                    ...
                    if EVALUATION_METHOD == 1:
                        ...
                        ssim_score = float(
                            tf.reduce_mean(ssim_specific(targets, autoencoder_output, mask, max_val=1.0)))
                        return autoencoder_loss, l1_loss, l2_loss, ssim_score
                except:
        ...
        #     f.write(
        #         f"{timestamp},{best_epoch},{l1_loss_train},{l1_loss_val},{l1_loss_test},{l1_loss_test_without_dropout},{l2_loss_train},{l2_loss_val},{l2_loss_test},{l2_loss_test_without_dropout},{ssim_train},{ssim_val},{final_test_performance['ssim_score']},{final_test_performance_without_dropout['ssim_score']}\n")
